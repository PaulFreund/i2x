...
function Handler(_events, _config) {
    ...
    // Methods
        ...
        onXMPP = function(type) {
                    ...
                    case 'useradd':
                        ...
                        getUserByJID(jid, function(key, user) {
                            if(user) {
                                ...
                                events.emit('store', 'set', 'users.'+key, {"jid": jid});
            ...
            case 'subscribe':
                var from = arguments[1];
                getUserByJID(from, function(user) {
                    if( user !== undefined )
                        events.emit('xmpp', 'subscribe', from);
                });
                break;
            ...
            case 'unsubscribe':
                ...
                getUserByJID(from, function(user) {
                    if( user !== undefined )
                        events.emit('xmpp', 'unsubscribe', from);
                });
                break;
